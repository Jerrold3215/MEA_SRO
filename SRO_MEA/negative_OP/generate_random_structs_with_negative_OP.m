...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if OP_FCC(f,4)<0 && OP_FCC(f,2)>0 && OP_ISF(f,4)<0 && OP_ISF(f,2)>0 && (OP_ISF(f,4)-OP_FCC(f,4))<0 && (OP_ISF(f,2)-OP_FCC(f,2))>0
    nOP_FCC(q,1)=OP_aa_FNN_FCC;
    nOP_FCC(q,2)=OP_bb_FNN_FCC;
    nOP_FCC(q,3)=OP_cc_FNN_FCC;
    nOP_FCC(q,4)=OP_ab_FNN_FCC;
    nOP_FCC(q,5)=OP_bc_FNN_FCC;
    nOP_FCC(q,6)=OP_ac_FNN_FCC;
    nOP_FCC(q,7)=OP_aa_SNN_FCC;
    nOP_FCC(q,8)=OP_bb_SNN_FCC;
    nOP_FCC(q,9)=OP_cc_SNN_FCC;
    nOP_FCC(q,10)=OP_ab_SNN_FCC;
    nOP_FCC(q,11)=OP_bc_SNN_FCC;
    nOP_FCC(q,12)=OP_ac_SNN_FCC;
    nOP_FCC(q,13)=OP_aa_TNN_FCC;
    nOP_FCC(q,14)=OP_bb_TNN_FCC;
    nOP_FCC(q,15)=OP_cc_TNN_FCC;
    nOP_FCC(q,16)=OP_ab_TNN_FCC;
    nOP_FCC(q,17)=OP_bc_TNN_FCC;
    nOP_FCC(q,18)=OP_ac_TNN_FCC;
    nOP_ISF(q,1)=OP_aa_FNN_ISF;
    nOP_ISF(q,2)=OP_bb_FNN_ISF;
    nOP_ISF(q,3)=OP_cc_FNN_ISF;
    nOP_ISF(q,4)=OP_ab_FNN_ISF;
    nOP_ISF(q,5)=OP_bc_FNN_ISF;
    nOP_ISF(q,6)=OP_ac_FNN_ISF;
    nOP_ISF(q,7)=OP_aa_SNN_ISF;
    nOP_ISF(q,8)=OP_bb_SNN_ISF;
    nOP_ISF(q,9)=OP_cc_SNN_ISF;
    nOP_ISF(q,10)=OP_ab_SNN_ISF;
    nOP_ISF(q,11)=OP_bc_SNN_ISF;
    nOP_ISF(q,12)=OP_ac_SNN_ISF;
    nOP_ISF(q,13)=OP_aa_TNN_ISF;
    nOP_ISF(q,14)=OP_bb_TNN_ISF;
    nOP_ISF(q,15)=OP_cc_TNN_ISF;
    nOP_ISF(q,16)=OP_ab_TNN_ISF;
    nOP_ISF(q,17)=OP_bc_TNN_ISF;
    nOP_ISF(q,18)=OP_ac_TNN_ISF;
  fid=fopen(['POSCAR-',int2str(q)],'wt');
  fprintf(fid,'%s\n','POSCAR');
  fprintf(fid,'%s\n','1.00000000000000');
...
fclose(fid);
  fid=fopen(['POSCAR-ISF-',int2str(q)],'wt');
  fprintf(fid,'%s\n','POSCAR');
  fprintf(fid,'%s\n','1.00000000000000');
  fprintf(fid,'%s\n','10.1823377609         0.0000000000         0.0000000000');
  fprintf(fid,'%s\n','5.0911688805         8.8181631709         0.0000000000');
 ...
 for i=1:1:m
    for j=1:1:n
       if j==n
        ...
        fprintf(fid,'%g\t',nOP_FCC(i,j));
       end
    end
end
fclose(fid);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  fid=fopen('negative_ISF_SRO','wt');
  fprintf(fid,'%s\n','Faa Fbb Fcc Fab Fbc Fac Saa Sbb Scc Sab Sbc Sac Taa Tbb Tcc Tab Tbc Tac');
  [m,n]=size(nOP_ISF);
 for i=1:1:m
    for j=1:1:n
